...
export async function GET(request: NextRequest) {
  try {
    ...
    const showActive =
      request.nextUrl.searchParams.get("showActive") === "true";
    ...
    if (
      user.role.name.toLowerCase() === "asociado" ||
      user.role.name.toLowerCase() === "staff"
    ) {
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          error instanceof Error ? error.message : "Error al cargar empresas",
      },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    try {
      const result = await prisma.$transaction(async (tx) => {
        ...
        // Obtener el rol del usuario para determinar si debe vincularse automáticamente
        const user = await tx.user.findUnique({
          where: { id: userId },
          include: { role: true },
        });

        // Si el usuario es Admin o Asociado, crear automáticamente la vinculación
        if (
          user &&
