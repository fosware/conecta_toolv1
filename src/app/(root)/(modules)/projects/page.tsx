...
import ProjectOverview, { ProjectOverviewRef } from "./components/project-overview";
...
export default function ProjectsPage() {
  ...
  const isAdmin = role?.toLowerCase() === 'admin' || role?.toLowerCase() === 'gerente';
  ...
  const associatedProjectRefs = useRef<Map<number, ProjectOverviewRef>>(new Map());
  ...
  const handleProjectStatusChange = useCallback((projectId: number, progress: number, status: string) => {
    ...
    setGroupedProjects(prevGroupedProjects => {
      ...
      const updatedGroups = prevGroupedProjects.map(groupedProject => {
        ...
        const projectToUpdate = groupedProject.asociados.find(asociado => asociado.id === projectId);
        
        if (projectToUpdate) {
          // Verificar si realmente hay un cambio de estado O progreso
          const statusChanged = projectToUpdate.projectStatusId !== newStatusId;
          ...
          const updatedAsociados = groupedProject.asociados.map(asociado => {
  ...
  const loadProjects = useCallback(async (showLoadingIndicator = true) => {
    try {
      ...
      if (isAdmin) {
        ...
        const groupedWithProgress = (data.items || []).map((group: GroupedProject) => ({
          ...group,
          asociados: group.asociados.map(asociado => ({
            ...asociado,
            realProgress: asociado.realProgress ?? (
              asociado.projectStatusId === 3 ? 100 : // Completado
              asociado.projectStatusId === 2 ? 50 :  // En progreso
              0 // Por iniciar
            )
          }))
        }));
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="mt-4">
                ...
                ) : (isAdmin ? groupedProjects.length === 0 : projects.length === 0) ? (
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className={isAdmin ? "w-[280px]" : ""}>Proyecto</TableHead>
                          {isAdmin ? (
                            // Header para ADMIN
                            <>
                              <TableHead className="w-[320px]">Progreso General</TableHead>
                              <TableHead className="w-[160px]">Fecha de Creación</TableHead>
                              <TableHead className="text-center w-[100px]">Asociados</TableHead>
                            </>
                          ) : (
                              ...
                              <TableRow>
                                <TableCell>
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() => {
                                      if (expandedProjectId === groupedProject.id) {
                                        setExpandedProjectId(null);
                                      } else {
                                        setExpandedProjectId(groupedProject.id);
                                      }
                                    }}
                                    title={
                                ...
                                <TableCell className="font-medium">{groupedProject.projectRequestTitle}</TableCell>
                                <TableCell>
                                  {(() => {
                                    ...
                                    const totalProgress = groupedProject.asociados.reduce((acc, asociado) => {
                                      ...
                                      const progress = asociado.realProgress ?? (
                                        asociado.projectStatusId === 3 ? 100 : // Completado
                                        asociado.projectStatusId === 2 ? 50 :  // En progreso
                                        0 // Por iniciar
                                      );
                                    ...
                                    const progresoPromedio = Math.round(totalProgress / totalAsociados);
                                ...
                                <TableCell>
                                  {formatDateForDisplay(groupedProject.createdAt)}
                                </TableCell>
                                ...
                                <TableRow>
                                  <TableCell colSpan={5} className="p-0">
                                    <div className="bg-muted/50 p-6 space-y-6">
                                      <h2 className="text-xl font-bold text-center mb-6">{groupedProject.projectRequestTitle}</h2>
                                      {groupedProject.asociados.map((asociado) => (
                                        <div key={asociado.id} className="border rounded-lg p-4 bg-background">
                                          <div className="mb-4 flex items-center justify-between">
                                            <h3 className="text-lg font-semibold">
                                              {getCompanyName(asociado.ProjectRequestCompany?.Company)}
                                            </h3>
                                            <div className="flex gap-2">
                                              ...
                                              <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={() => setProjectForLogs(asociado)}
                                                title="Ver bitácora"
                                                className="h-8 px-2 relative"
                                              >
                                                <MessageSquare className="h-4 w-4" />
                                                <UnreadIndicator projectId={asociado.id} />
                                              </Button>
                                            </div>
                                          </div>
                                          <ProjectOverview
                                            ref={getAssociatedProjectRef(asociado.id)}
                                  ...
                                  <TableCell className="text-right">
                                    <div className="flex items-center justify-end gap-2">
                                      ...
                                      <Button
                                        variant="outline"
                                        size="sm"
                                        onClick={() => setProjectForLogs(project)}
                                        title="Ver bitácora"
                                        className="h-8 px-2 relative"
                                      >
                                        <MessageSquare className="h-4 w-4" />
                                  ...
                                  <TableRow>
                                    <TableCell colSpan={6} className="p-0">
                                      <div className="bg-muted/50 p-6">
                                        <ProjectOverview
                                          ref={expandedProjectId === project.id ? projectOverviewRef : null}
      ...
      {selectedProject && (
        <ProjectCategoriesModal
          ...
          onSuccess={async () => {
            ...
            const associatedRef = associatedProjectRefs.current.get(selectedProject.id);
            ...
            if (expandedProjectId === selectedProject.id && projectOverviewRef.current) {
